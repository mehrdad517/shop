...
import TicketCategoryCreate from "./create";
import Dialog from "@material-ui/core/Dialog";
import TicketCategoryEdit from "./edit";
import EditIcon from "@material-ui/icons/Edit";
import TicketCategoryHead from "./head";
...
class TicketCategory extends Component {
    ...
    async handleRequest() {
        ...
        await new Promise((resolve => {
            resolve(instance.getTicketCategories(false).then((response) => {
                ...
                <Container>
                    ...
                    {!this.state.loading ? <CircularProgress color={"secondary"} /> : <div>
                        ...
                        <Dialog open={this.state.dialog}  onClose={() => this.setState({dialog: false})}>
                            {this.state.checked.length > 0 && <TicketCategoryEdit entity={this.state.checked[0]}  handleRequest={() => this.handleRequest()} onClose={() => this.setState({dialog: false})} />}
                        </Dialog>
                    </div>}
                </Container>
            </div>
