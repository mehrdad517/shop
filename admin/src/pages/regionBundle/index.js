...
class Region extends Component {
    ...
    async handleRequest() {
        ...
        await new Promise((resolve => {
            resolve(instance.getRegions(false).then((response) => {
                    ...
                    {!this.state.loading ? <CircularProgress color={"secondary"} /> : <div>
                        <Box>
                            <div style={{ display: 'flex', direction: 'row', justifyContent: 'flex-end'}}>
                                {Boolean(this.props.auth.permissions.region.store.access) && <RegionCreate handleRequest={this.handleRequest.bind(this)}  items={this.state.checked} />}
                                {Boolean(this.props.auth.permissions.region.update.access) && <Tooltip title="ویرایش">
                                ...
                                {Boolean(this.props.auth.permissions.region.index.access) && <Tooltip title="سینک">
                        ...
                        <Dialog fullWidth={true} open={this.state.dialog}  onClose={() => this.setState({dialog: false})}>
                            {this.state.checked.length > 0 && <RegionEdit entity={this.state.checked[0]}  handleRequest={() => this.handleRequest()} onClose={() => this.setState({dialog: false})} />}
                        </Dialog>
                    </div>}
                </Container>
            </div>
