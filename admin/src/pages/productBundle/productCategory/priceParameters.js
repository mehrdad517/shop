...
class ProductCategoryPriceParameters extends Component {
    ...
    handleRequest() {
        ...
        this.api.getProductCategoryPriceParameters(this.props.match.params.id).then((response) => {
           if (typeof response != "undefined") {
               this.setState({
                   chipList: response
               });
        ...
        this.api.getAllPriceParametersWithChecked(this.props.match.params.id, {page: this.state.page, title: this.state.title}).then((response) => {
            if (typeof response != "undefined") {
                this.setState({
                    entities: response,
                    loading: false
    ...
    chipDeleteable(id) {
        ...
        instance.storeProductCategoryPriceParameters(this.props.match.params.id, {
            price_parameter: id
        }).then((response) => {
            if (typeof response != "undefined") {
                if (response.status) {
    ...
    handleChangeAttr(event) {
        ...
        let instance = new Api();
        instance.storeProductCategoryPriceParameters(this.props.match.params.id, {
            price_parameter: event.target.value
        }).then((response) => {
            if (typeof response != "undefined") {
                if (response.status) {
                    this.setState({
                    ...
                    <Box style={{ margin: '10px 0 20px 0', borderRadius: '30px'}}>
                        <Grid container alignItems="center">
                            <Grid item xs={12} sm={6}>
                                ...
                                <p style={{ color: '#8e8e8e'}}>در این صفحه میتوانید  برای هر دسته بندی پارامترهای قیمتی را تغییر دهید.</p>
...
export default connect(
    mapStateToProps,
    mapDispatchToProps
)(ProductCategoryPriceParameters);
