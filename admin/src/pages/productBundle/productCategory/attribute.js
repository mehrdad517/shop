...
class ProductCategoryAttribute extends Component {
    ...
    handleRequest() {
        ...
        this.api.getProductCategoryAttributes(this.props.match.params.id).then((response) => {
           if (typeof response != "undefined") {
               this.setState({
                   chipList: response
               });
        ...
        this.api.getAllAttributesWithChecked(this.props.match.params.id, {page: this.state.page, title: this.state.title}).then((response) => {
            if (typeof response != "undefined") {
                this.setState({
                    entities: response,
                    loading: false
            ...
            <div className={'content'}>
                <Container>
                    <Box style={{ margin: '10px 0 20px 0', borderRadius: '30px'}}>
                        <Grid container alignItems="center">
                            <Grid item xs={12} sm={6}>
                                ...
                                <p style={{ color: '#ff5b60'}}>کلیه زیر شاخه ها ویژگیها را به ارث میبرند.</p>
                            </Grid>
                            <Grid item xs={12} sm={6} >
                    ...
                    <Box style={{ display: 'flex', flexDirection: 'row', justifyContent: 'flex-end'}}>
                        <AttributeCreate handleRequest={this.handleRequest.bind(this)} />
                    </Box>
                    <div style={{ position: 'relative'}}>
                        <Box boxShadow={2} style={{ backgroundColor: (!this.state.loading ? '#fff' : 'rgb(232,232,232)'), padding: '25px', borderRadius: '7px'}}>
                            <Grid container>
